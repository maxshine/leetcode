...
def input_with_varify(desc: str = None):
    ...
    try:
        in_str = input(desc)
        if os.path.exists(in_str) and in_str.lower().endswith('.epub'):
            in_flag = False
    except:
        pass
    if in_flag:
    ...
    with tqdm.tqdm(total=file_num, desc='正在准备去除drm保护', leave=False) as show:
        for dirpath, dirnames, filenames in os.walk(tmp_dir):
            for filepath in filenames:
                file_names_old.append(
                    filepath.replace('0', '*').replace('1', ':'))
                file_names_old_url.append(
                    filepath.replace('0', '%2A').replace('1', '%3A'))
    ...
    with tqdm.tqdm(total=file_num, desc='正在去除drm保护', leave=False) as show:
        for file_path in file_paths:
            if file_path.endswith('.xhtml') \
                ...
                new_lines = []
                for line in lines:
                    for file_name_old, file_name_old_url, file_name_new in zip(file_names_old, file_names_old_url, file_names_new):
                        line = line.replace(file_name_old, file_name_new).replace(
                            file_name_old_url, file_name_new)
...
def main():
    print_info()
    tmp_dir = r'.\no_drm_epub'
    os.system('rd /s /q {}'.format(tmp_dir))
